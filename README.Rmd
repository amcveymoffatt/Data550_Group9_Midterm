...
Two versions of the report can be generated. The cox model in table 3 can be altered through the use of the `config.yml` file. Depending on the version, the second variable in the model will be either: 1. Breastfed Status 2. Primary Caregiver

### Requirements

-   `Packages`: Includes here, tidyverse, survival, and gtsummary.
    -   Missing packages are identified and installed in `00_renv.R` using the renv package.
-   `R`
-   `Make`
-   `Git` (optional)
...
3.  Set which version of the report to generate by running the following commands in the terminal
...
5.  Remove all files generated when running `Make report_${WHICH_CONFIG}.html`
